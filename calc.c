...
char main_options() {
	...
	while (1) {
		if (option == 'B' || option == 'U' || option == 'A' || option == 'V' || option == 'E') {
			...
			printf("Invalid operation selected, try again ( B , U , A , V , E )\n");
			scanf(" %1c", &option);
			int c;
			while ((c = getchar()) != '\n' && c != EOF) { }
		}
